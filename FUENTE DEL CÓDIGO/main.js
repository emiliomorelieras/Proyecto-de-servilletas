...
function add() {
	...
	background_imgTag.onload = uploadBackground; // establece la funciÃ³n para cargar esta variable
	background_imgTag.src = background_image;   // carga la imagen

	rover_imgTag = new Image(); //define una variable para la imagen nueva
	rover_imgTag.onload = uploadrover; // establece la funciÃ³n para cargar esta variable
...
function up(){
	...
	{
		rover_y = rover_y - 10;
		console.log("When up, x=" + rover_x + "y-" + rover_y);
		 uploadBackground();
		 uploadrover();
	} 
}
